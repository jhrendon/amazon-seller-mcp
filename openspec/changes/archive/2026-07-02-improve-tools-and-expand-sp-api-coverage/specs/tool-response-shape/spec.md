...
### Requirement: structuredContent is strongly typed

The `structuredContent` value SHALL be typed according to the specific payload shape returned by the handler. Handlers SHALL NOT cast the payload to `Record<string, unknown>` or `unknown` solely to satisfy the SDK; numeric and boolean fields SHALL be passed as their native types.
...
#### Scenario: No broad casting
- **WHEN** inspecting handler source code
- **THEN** `as unknown as Record<string, unknown>` is not used to produce `structuredContent`

## ADDED Requirements

### Requirement: Shared response helper enforces typing

The system SHALL provide `makeToolResponse<T>(payload: T)` that returns the correct `{ content, structuredContent }` shape while preserving the generic type `T`.

#### Scenario: Using the helper
- **WHEN** a handler calls `makeToolResponse({ totalOrders: 42 })`
- **THEN** TypeScript infers `structuredContent` as `{ totalOrders: number }`
- **AND** the runtime output matches the previous hand-written response shape
